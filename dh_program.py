...
class SecurePRNG:
    ...
    def xor_crypt(data, prng):
        ...
        return bytes(a ^ b for a, b in zip(data, prng.generate(len(data))))
...
class Entity:
    ...
    # Calculates and initializes shared secret with SecurePRNG
...
# This class simulates the network and allows for an interceptor 'hook' (Mallory) to manipulate 
# messages in transit.
class Network:
    def __init__(self):
...
class Mallory:
    ...
    def intercept(self, sender, recipient, payload):
        ...
        if isinstance(payload, bytes):
            ...
            if sender == "Alice":
                # Decrypts the message using the appropriate session PRNG
                decrypted = SecurePRNG.xor_crypt(payload, self.alice_prng)
            ...
            # Additonal logic for Bob -> Alice (not required but I decided to implement)
            if sender == "Bob":
                # Decrypts the message using the appropriate session PRNG
                decrypted = SecurePRNG.xor_crypt(payload, self.bob_prng)
                # Prints the plaintext message to the console for Mallory's spying purposes
                print(f"Decrypted Message: {decrypted.decode()}")
                # Modifies the plaintext message in some way
                modified = decrypted.replace(b"9pm", b"3am")
...
def main():
    ...
    message = b"The Dragons have gone flying." # Put in your test message here
